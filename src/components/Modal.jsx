...
function Modal({show,onClose,title,emoji,children,wide,xl}){
  if(!show) return null;
  const minW = xl ? 620 : wide ? 520 : 360;
  const maxW = xl ? 760 : wide ? 640 : 500;
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,background:"rgba(45,45,58,.35)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,backdropFilter:"blur(6px)"}}>
      <div onClick={e=>e.stopPropagation()} className="card-appear" style={{background:"var(--paper)",borderRadius:"var(--r-lg)",boxShadow:"var(--shadow-lg)",padding:28,minWidth:minW,maxWidth:maxW,width:"92%",border:"2px solid var(--border)",maxHeight:"90vh",overflowY:"auto"}}>
